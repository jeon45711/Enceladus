...
C++ Constructor Array: Maps .init_array section to sequentially invoke global object constructors.

Provided Symbols: Defines __global_pointer$, __bss_start, and _end for Newlib _sbrk heap memory management.

System Call Stubs (c_runtime_header)
_write: Directs serial character output directly to UART0 MMIO registers.

_sbrk: Allocates dynamic memory starting from the _end symbol boundary.
